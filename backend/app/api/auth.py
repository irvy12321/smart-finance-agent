...
async def register(request: Request, user_data: UserCreate):
    ...
    try:
        ...
        logger.info(f"User created: id={user['id']}, username={user['username']}, role={user_role}")
...
async def refresh_token(request: Request, body: RefreshTokenRequest):
    ...
    try:
        ...
        logger.info(f"Token refreshed successfully: user_id={user['id']}, role={user_role}")
...
async def admin_create_user(
    ...
    """
    try:
        logger.info(f"Admin create user attempt: username={user_data.username}, role={user_data.role} by {current_user.username}")
        ...
        logger.info(f"Admin created user: {user_data.username} (role={user_data.role}) by {current_user.username}")
...
async def admin_update_user(
    ...
    try:
        ...
        try:
            # 检查用户是否存在
            cursor = conn.execute("SELECT id, username, email, role, is_active, created_at FROM users WHERE id = ?", (user_id,))
            ...
            logger.info(f"Admin updated user {user_id}: {user_data.dict(exclude_none=True)} by {current_user.username}")
...
async def admin_delete_user(
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Admin deleted user: {user[1]} (id={user_id}) by {current_user.username}")
